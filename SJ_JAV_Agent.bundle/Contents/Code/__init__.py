...
class SJ_JAV_Agent(Agent.Movies):
	...
	def search(self, results, media, lang, manual=False):
		...
		url = '%s?mode=search&arg=%s' % (Prefs['server'], (urllib.quote(media.name.encode('utf8'))))
		...
		for item in data:
			...
			year = media.year
			Log.Debug('ID=%s, media.name=%s, title=%s, year=%s, score=%d' %(id, media.name, title, year, score))
